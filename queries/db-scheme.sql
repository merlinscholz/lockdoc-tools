...
CREATE TABLE `structs_layout` (
  ...
  `size` int(11) UNSIGNED NOT NULL,	-- The size in bytes of a member
...
CREATE TABLE `function_blacklist` (			-- A per datatype list of blacklisted functions. We want to ignore memory accesses from these functions.
  ...
  `sequence` int(11) DEFAULT NULL,		-- The position in the stack trace to which an entry should be applied to
  PRIMARY KEY (`id`),
